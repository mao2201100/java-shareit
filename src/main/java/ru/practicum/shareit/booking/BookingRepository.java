...
//    List<Booking> findByBookerId(Long bookerId);
    ...
    @Query(value = "select * from booking book inner join item i on i.id = book.item_id where item_id = :itemId and i.owner_id = :ownerId and book.status = 'APPROVED' order by book.id desc limit 1", nativeQuery = true)
    Booking fetchLastBookerByItem(long itemId, long ownerId);

    @Query(value = "select * from booking book where item_id = :itemId and book.id not in (select bookSub1.id from booking bookSub1  inner join item iSub1 on iSub1.id = bookSub1.item_id where bookSub1.status = 'APPROVED' and bookSub1.item_id = :itemId and iSub1.owner_id = :ownerId and bookSub1.status = 'APPROVED' order by bookSub1.id desc limit 1) and exists((select bookSub1.id from booking bookSub1  inner join item iSub1 on iSub1.id = bookSub1.item_id where bookSub1.item_id = :itemId and iSub1.owner_id = :ownerId and bookSub1.status = 'APPROVED' order by bookSub1.id desc limit 1)) = true and book.booker_id not in (select bookSub2.booker_id from booking bookSub2  inner join item isub2 on isub2.id = bookSub2.item_id where bookSub2.item_id = :itemId and isub2.owner_id = :ownerId  and bookSub2.status = 'APPROVED' order by bookSub2.id desc limit 1) order by book.id desc limit 1", nativeQuery = true)
    Booking fetchNextBookerByItem(long itemId, long ownerId);


    @Query(value = "select * from booking book inner join item i on i.id = book.item_id where item_id = :itemId and i.owner_id = :ownerId and book.status = 'APPROVED' order by book.id asc limit 1", nativeQuery = true)
    Booking fetchLastBookerByItemForAll(long itemId, long ownerId);

    @Query(value = "select * from booking book where item_id = :itemId and book.id not in (select bookSub1.id from booking bookSub1  inner join item iSub1 on iSub1.id = bookSub1.item_id where bookSub1.status = 'APPROVED' and bookSub1.item_id = :itemId and iSub1.owner_id = :ownerId and bookSub1.status = 'APPROVED' order by bookSub1.id asc limit 1) and exists((select bookSub1.id from booking bookSub1  inner join item iSub1 on iSub1.id = bookSub1.item_id where bookSub1.item_id = :itemId and iSub1.owner_id = :ownerId and bookSub1.status = 'APPROVED' order by bookSub1.id asc limit 1)) = true and book.booker_id not in (select bookSub2.booker_id from booking bookSub2  inner join item isub2 on isub2.id = bookSub2.item_id where bookSub2.item_id = :itemId and isub2.owner_id = :ownerId  and bookSub2.status = 'APPROVED' order by bookSub2.id asc limit 1) order by book.id asc limit 1", nativeQuery = true)
    ...
    @Query(value = "select * from booking book where book.item_id = :itemId and book.booker_id = :bookerId and DATE(book.end_date) < now() and book.status = 'APPROVED'", nativeQuery = true)
